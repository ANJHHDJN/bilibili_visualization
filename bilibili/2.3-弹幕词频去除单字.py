      ...
      for file in files:
         ...
         for i in data['弹幕']:
            if len(i) > 1:
                danmu_cut.append(i)
      stopwords = open('stopwords.txt', 'r', encoding='utf-8').read()
      words_dict2 = statistics(danmu_cut, stopwords)
      save(words_dict2, savename=filename + '词频表', savapath='./词频表(去单字)/')
      drawWordCloud(words_dict2, filename + '词云图', savepath='./词云图(去单字)/')
